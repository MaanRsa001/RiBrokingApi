...
import com.maan.insurance.model.req.facultative.ShowFirstPageReq;
...
import com.maan.insurance.model.res.facultative.ShowFirstPageRes;
import com.maan.insurance.model.res.facultative.ShowFirstPageRes1;
...
//			}
	...
	public String[] firstPageInsertAruguments(FirstPageInsertReq beanObj,final boolean flag,final boolean contract) throws ParseException {
		...
		if((StringUtils.isBlank(beanObj.getProposalNo())?"":beanObj.getProposalNo()).equalsIgnoreCase("")){
				args[0]= fm.getSequence("Proposal", beanObj.getProductId(),beanObj.getDepartmentId(), beanObj.getBranchCode(),(StringUtils.isBlank(beanObj.getProposalNo())?"":beanObj.getProposalNo()),"");
		}
		else
	...
	public CommonResponse insertRemarkDetails(RemarksSaveReq beanObj) {
		...
		try {
			...
			for(int i=0;i<beanObj.getRemarksReq().size();i++){
				...
				obj[1]=StringUtils.isBlank(beanObj.getContractNo())?"":beanObj.getContractNo();
				obj[2]=StringUtils.isBlank(beanObj.getLayerNo())?"0":beanObj.getLayerNo();
	...
	public SecondPageInsertRes secondPageInsert(SecondPageInsertReq beanObj) {
		...
		//	updateSecondPage(beanObj);
			if(result==1)
				...
				saveFlag=false;
			if("".equalsIgnoreCase(beanObj.getContractNo()==null?"":beanObj.getContractNo())||"0".equalsIgnoreCase(beanObj.getContractNo())){
						...
						if("A".equalsIgnoreCase(res.getProStatus()) && ! "0".equalsIgnoreCase(beanObj.getShSd())){
							if(res.getContractNo()==null||"0".equalsIgnoreCase(res.getContractNo())||"".equalsIgnoreCase(res.getContractNo())){
								...
								if(!"".equals(beanObj.getRenewalContractno())&&"OLDCONTNO".equals(beanObj.getRenewalFlag())){
									...
									res.setStatus("Your Proposal is converted to Contract with Proposal No : "+beanObj.getProposalNo() +" and Contract No : "+res.getContractNo()+".");
								}
								res.setBackmode("Con");
							}
							else{
								res.setStatus("Your Contract is updated with Proposal No : "+beanObj.getProposalNo()+" and Contract No : "+res.getContractNo()+".");
								res.setBackmode("Con");
							}
...
//			}
	...
	@Override
	public ShowFirstPageRes showFirstPageItems(ShowFirstPageReq req) {
		ShowFirstPageRes response = new ShowFirstPageRes();
		ShowFirstPageRes1 res = new ShowFirstPageRes1();
		List<ShowFirstPageRes1> resList = new ArrayList<ShowFirstPageRes1>();
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		try {
			String query;
			...
			if(list!=null && list.size()>0){
				...
				res.setEndorsmentno(tempMap.get("RSK_ENDORSEMENT_NO")==null?"":tempMap.get("RSK_ENDORSEMENT_NO").toString());
				res.setProductId(tempMap.get("RSK_PRODUCTID")==null?"":tempMap.get("RSK_PRODUCTID").toString());
				res.setDepartmentId(tempMap.get("RSK_DEPTID")==null?"":tempMap.get("RSK_DEPTID").toString());
				res.setProfitCenterCode(tempMap.get("RSK_PFCID")==null?"":tempMap.get("RSK_PFCID").toString());
				res.setSubProfitCenter(tempMap.get("RSK_SPFCID")==null?"":tempMap.get("RSK_SPFCID").toString());
				res.setCedingCompany(tempMap.get("RSK_CEDINGID")==null?"":tempMap.get("RSK_CEDINGID").toString());
				res.setBroker(tempMap.get("RSK_BROKERID")==null?"":tempMap.get("RSK_BROKERID").toString());
				res.setMonth(tempMap.get("MONTH")==null?"":tempMap.get("MONTH").toString());
				res.setYear(tempMap.get("RSK_UWYEAR")==null?"":tempMap.get("RSK_UWYEAR").toString());
				res.setUnderwriter(tempMap.get("RSK_UNDERWRITTER")==null?"":tempMap.get("RSK_UNDERWRITTER").toString());
				res.setInceptionDate(tempMap.get("INS_DATE")==null?"":(tempMap.get("INS_DATE").toString()));
				res.setExpiryDate(tempMap.get("EXP_DATE")==null?"":(tempMap.get("EXP_DATE").toString()));
				res.setAccountDate(tempMap.get("AC_DATE")==null?"":(tempMap.get("AC_DATE").toString()));
				res.setOriginalCurrency(tempMap.get("RSK_ORIGINAL_CURR")==null?"":tempMap.get("RSK_ORIGINAL_CURR").toString());
				res.setUsCurrencyRate(StringUtils.isBlank(res.getContractNo())&&(StringUtils.isBlank(tempMap.get("RSK_EXCHANGE_RATE")==null?"":tempMap.get("RSK_EXCHANGE_RATE").toString()))?"":dropDowmImpl.exchRateFormat(tempMap.get("RSK_EXCHANGE_RATE")==null?"":tempMap.get("RSK_EXCHANGE_RATE").toString()));
				res.setTerritory(tempMap.get("RSK_TERRITORY")==null?"":tempMap.get("RSK_TERRITORY").toString());
				res.setInsuredName(tempMap.get("RSK_INSURED_NAME")==null?"":tempMap.get("RSK_INSURED_NAME").toString());
				res.setLocation(tempMap.get("RSK_LOCATION")==null?"":tempMap.get("RSK_LOCATION").toString());
				res.setCity(tempMap.get("RSK_CITY")==null?"":tempMap.get("RSK_CITY").toString());
				res.setNr(StringUtils.isBlank(res.getContarctno())?"":tempMap.get("NR")==null?"":tempMap.get("NR").toString());
				res.setCedantsRet(tempMap.get("RSK_CEDANT_RETENTION")==null?"":tempMap.get("RSK_CEDANT_RETENTION").toString());
				res.setMaxiumlimit(tempMap.get("MAXIMUM_LIMIT_OC")==null?"":tempMap.get("MAXIMUM_LIMIT_OC").toString());
				res.setDeductible(tempMap.get("DEDUCTIBLE_OC")==null?"":tempMap.get("DEDUCTIBLE_OC").toString());
				res.setSpRetro(StringUtils.isBlank(tempMap.get("SP_RETRO")==null?"":tempMap.get("SP_RETRO").toString())?"N":tempMap.get("SP_RETRO").toString());
				res.setPml(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("PML")==null?"":tempMap.get("PML").toString()))?"":tempMap.get("PML").toString());
				res.setSipml(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("SI_PML_OC")==null?"":tempMap.get("SI_PML_OC").toString()))?"SI":tempMap.get("SI_PML_OC").toString());
				res.setCu(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("CU")==null?"":tempMap.get("CU").toString()))?"":tempMap.get("CU").toString());
				res.setCuRsn(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("CU_RSN")==null?"":tempMap.get("CU_RSN").toString()))?"":tempMap.get("CU_RSN").toString());
				res.setSumInsured(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("SUM_INSURED_OC")==null?"":tempMap.get("SUM_INSURED_OC").toString()))?"":tempMap.get("SUM_INSURED_OC").toString());
				res.setGwpi(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("GWPI_OC")==null?"":tempMap.get("GWPI_OC").toString()))?"":tempMap.get("GWPI_OC").toString());
				res.setPmll(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("PML_OC")==null?"":tempMap.get("PML_OC").toString()))?"":tempMap.get("PML_OC").toString());
				res.setTpl(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("TPL_OC")==null?"":tempMap.get("TPL_OC").toString()))?"":fm.formatter(tempMap.get("TPL_OC").toString()));
				res.setShWt(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("SHARE_WRITTEN")==null?"":tempMap.get("SHARE_WRITTEN").toString()))?"":tempMap.get("SHARE_WRITTEN").toString());
				res.setShSd(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(tempMap.get("SHARE_SIGNED")==null?"":tempMap.get("SHARE_SIGNED").toString())||"0".equalsIgnoreCase(tempMap.get("SHARE_SIGNED")==null?"":tempMap.get("SHARE_SIGNED").toString()))?"":tempMap.get("SHARE_SIGNED").toString());
				res.setProStatus(tempMap.get("RSK_STATUS")==null?"":tempMap.get("RSK_STATUS").toString());
				res.setNoInsurer((StringUtils.isBlank(tempMap.get("SP_RETRO")==null?"":tempMap.get("SP_RETRO").toString())||"N".equalsIgnoreCase(tempMap.get("SP_RETRO").toString()))?"0":tempMap.get("NO_OF_INSURERS")==null?"":tempMap.get("NO_OF_INSURERS").toString());
				res.setPremiumrate(StringUtils.isBlank(res.getContarctno())&&(StringUtils.isBlank(res.getNoInsurer()))?"":tempMap.get("RSK_PREMIUM_RATE")==null?"":tempMap.get("RSK_PREMIUM_RATE").toString());
				res.setPolicyBranch(tempMap.get("RSK_POLBRANCH")==null?"":tempMap.get("RSK_POLBRANCH").toString());
				res.setCedRetenType(StringUtils.isBlank(tempMap.get("RSK_CEDRET_TYPE").toString())?"A":tempMap.get("RSK_CEDRET_TYPE").toString());
				res.setRenewalContractno(tempMap.get("OLD_CONTRACTNO")==null?"":tempMap.get("OLD_CONTRACTNO").toString());
				res.setRenewalStatus(tempMap.get("RENEWAL_STATUS")==null?"1":tempMap.get("RENEWAL_STATUS").toString());
				res.setBaseLoginID(tempMap.get("LOGIN_ID")==null?"":tempMap.get("LOGIN_ID").toString());
				res.setDeductibleFacXol(tempMap.get("DEDUCTIBLE_FACXOL_OC")==null?"":tempMap.get("DEDUCTIBLE_FACXOL_OC").toString());
				res.setXolOC(tempMap.get("XOL_OC")==null?"":tempMap.get("XOL_OC").toString());
				String mdinst=tempMap.get("MND_INSTALLMENTS")==null?"":tempMap.get("MND_INSTALLMENTS").toString();
				res.setNoOfInst(StringUtils.isBlank(mdinst)||"0".equals(mdinst)?"1":mdinst);
				res.setModeOfTransport(tempMap.get("MODE_TRANSPORT_ID")==null?"":tempMap.get("MODE_TRANSPORT_ID").toString());
				res.setVesselName(tempMap.get("VESSEL_NAME")==null?"":tempMap.get("VESSEL_NAME").toString());
				res.setVesselAge(tempMap.get("VESSEL_AGE")==null?"":tempMap.get("VESSEL_AGE").toString());

				String lpvOC = tempMap.get("LIMIT_PER_VESSEL_OC")==null?"":tempMap.get("LIMIT_PER_VESSEL_OC").toString();
				String lplOC = tempMap.get("LIMIT_PER_LOCATION_OC")==null?"":tempMap.get("LIMIT_PER_LOCATION_OC").toString();

				res.setLimitPerVesselOC(fm.formatter(StringUtils.isBlank(res.getContractNo())&&(StringUtils.isBlank(lpvOC)||"0".equals(lpvOC))?"":lpvOC));
				res.setLimitPerLocationOC(fm.formatter(StringUtils.isBlank(res.getContractNo())&&(StringUtils.isBlank(lplOC)||"0".equals(lplOC))?"":lplOC));

				res.setType(tempMap.get("TYPE")==null?"":tempMap.get("TYPE").toString());
				res.setInterest(tempMap.get("RSK_INTEREST")==null?"":tempMap.get("RSK_INTEREST").toString());
				res.setCountryIncludedList(tempMap.get("COUNTRIES_INCLUDE")==null?"":tempMap.get("COUNTRIES_INCLUDE").toString());
				res.setCountryExcludedList(tempMap.get("COUNTRIES_EXCLUDE")==null?"":tempMap.get("COUNTRIES_EXCLUDE").toString());
				res.setContractListVal(tempMap.get("DATA_MAP_CONT_NO")==null?"":tempMap.get("DATA_MAP_CONT_NO").toString());
				res.setDeductibleFacXolPml(tempMap.get("COVER_LIMIT_PML_OC")==null?"":tempMap.get("COVER_LIMIT_PML_OC").toString());
				res.setGwpiPml(tempMap.get("GWPI_PML_OC")==null?"":tempMap.get("GWPI_PML_OC").toString());
				res.setPslOC(tempMap.get("PSL_OC")==null?"":tempMap.get("PSL_OC").toString());
				res.setPllOC(tempMap.get("PLL_OC")==null?"":tempMap.get("PLL_OC").toString());
				res.setPblOC(tempMap.get("PBL_OC")==null?"":tempMap.get("PBL_OC").toString());
				res.setSumInsuredPml(tempMap.get("SUM_INS_PML_OC")==null?"":tempMap.get("SUM_INS_PML_OC").toString());
				res.setInwardType(tempMap.get("INWARD_BUS_TYPE")==null?"":tempMap.get("INWARD_BUS_TYPE").toString());
				res.setReceiptofPayment(tempMap.get("RSK_RECEIPT_PAYEMENT")==null?"":tempMap.get("RSK_RECEIPT_PAYEMENT").toString());
				res.setLocIssued(tempMap.get("RSK_LOC_ISSUED")==null?"":tempMap.get("RSK_LOC_ISSUED").toString());
				res.setLatitude(tempMap.get("RSK_LATITUDE")==null?"":tempMap.get("RSK_LATITUDE").toString());
				res.setLongitude(tempMap.get("RSK_LONGITUDE")==null?"":tempMap.get("RSK_LONGITUDE").toString());
				res.setVessaletonnage(tempMap.get("RSK_VESSAL_TONNAGE")==null?"":tempMap.get("RSK_VESSAL_TONNAGE").toString());
				res.setEndorsmenttype(tempMap.get("RS_ENDORSEMENT_TYPE")==null?"":tempMap.get("RS_ENDORSEMENT_TYPE").toString());
				res.setLocBankName(tempMap.get("RSK_LOC_BNK_NAME")==null?"":tempMap.get("RSK_LOC_BNK_NAME").toString());
				res.setLocCreditPrd(tempMap.get("RSK_LOC_CRDT_PRD")==null?"":tempMap.get("RSK_LOC_CRDT_PRD").toString());
				res.setLocCreditAmt(tempMap.get("RSK_LOC_CRDT_AMT")==null?"":fm.formatter(tempMap.get("RSK_LOC_CRDT_AMT").toString()));
				res.setLocBeneficerName(tempMap.get("RSK_LOC_BENFCRE_NAME")==null?"":tempMap.get("RSK_LOC_BENFCRE_NAME").toString());
				res.setXollayerNo(tempMap.get("Xol_LAYER_NO")==null?"":tempMap.get("Xol_LAYER_NO").toString());
				resList.add(res);
			}
			
			if(StringUtils.isNotBlank(res.getContractNo())&&!"0".equals(res.getContractNo())){
				res.setPrclFlag(dropDowmImpl.getPLCLCountStatus(res.getContractNo(), "0"));
			}else{
				res.setPrclFlag(false);
			}
			res.setAmendId(dropDowmImpl.getRiskComMaxAmendId(res.getProposalNo()));
						
//			getRemarksDetails(req.getProposalNo());
//			getCoverDeductableDetails(req.getProposalNo(),req.getBranchcode(),req.getProductId());
//			GetXolCoverDeductableDetails(req.getProposalNo(),req.getBranchcode());
