...
import com.maan.insurance.model.repository.TtrnAllocatedTransactionRepository;
		...
		try{
			...
			if("S".equalsIgnoreCase(bean.getSearchType())){
				if(StringUtils.isNotBlank(bean.getCompanyNameSearch())){
					qutext +=" AND UPPER(CUSTOMER_NAME) LIKE UPPER('%"+bean.getCompanyNameSearch()+"%')";
				}
				if(StringUtils.isNotBlank(bean.getBrokerNameSearch())){
					qutext +=" AND UPPER(BROKER_NAME) LIKE UPPER('%"+bean.getBrokerNameSearch()+"%')";
				}
				if(StringUtils.isNotBlank(bean.getContractNoSearch())){
					qutext +=" AND RSK_CONTRACT_NO LIKE ('%"+bean.getContractNoSearch()+"%')";
				}
				if(StringUtils.isNotBlank(bean.getTransactionNoSearch())){
					qutext +=" AND TRANSACTION_NO LIKE ('%"+bean.getTransactionNoSearch()+"%')";
				}
				if(StringUtils.isNotBlank(bean.getTransactionDateSearch())){
					qutext +=" AND TRANSACTION_DATE LIKE ('%"+bean.getTransactionDateSearch()+"%')";
				}
			...
			}else  if("Temp".equalsIgnoreCase(bean.getTableType())){
				if("ST".equalsIgnoreCase(bean.getSearchType())){
					if(StringUtils.isNotBlank(bean.getCompanyNameSearchTemp())){
						qutext +=" AND UPPER(CUSTOMER_NAME) LIKE UPPER('%"+bean.getCompanyNameSearchTemp()+"%')";
					}
					if(StringUtils.isNotBlank(bean.getBrokerNameSearchTemp())){
						qutext +=" AND UPPER(BROKER_NAME) LIKE UPPER('%"+bean.getBrokerNameSearchTemp()+"%')";
					}
					if(StringUtils.isNotBlank(bean.getContractNoSearchTemp())){
						qutext +=" AND RSK_CONTRACT_NO LIKE ('%"+bean.getContractNoSearchTemp()+"%')";
					}
					if(StringUtils.isNotBlank(bean.getTransactionNoSearchTemp())){
						qutext +=" AND REQUEST_NO LIKE ('%"+bean.getTransactionNoSearchTemp()+"%')";
					}
					if(StringUtils.isNotBlank(bean.getTransactionDateSearchTemp())){
						qutext +=" AND TRANSACTION_DATE LIKE ('%"+bean.getTransactionDateSearchTemp()+"%')";
					}
