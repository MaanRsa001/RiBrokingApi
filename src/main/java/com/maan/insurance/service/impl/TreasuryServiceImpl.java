	...
	public AllocateViewCommonRes1 allocateView(AllocateViewReq req) {
		...
		try {
			...
			if (!"".equals(req.getSerialNo()) && !"VIEW".equalsIgnoreCase(req.getFlag())) {
				...
				/*
				 * res.setAllTillDate(allocateView.get(0).get("ALLOCATED_TILL_DATE")==null?"":
				 * allocateView.get(0).get("ALLOCATED_TILL_DATE").toString());
				 * 
				 * bean.setAllTillDate((String)this.mytemplate.queryForObject(selectQry,
				 * selectArgs, String.class));
				 */
	...
	public GetReceiptReversalListRes1 getReceiptReversalList(GetReceiptReversalListReq req) {
		...
		try {
			...
			if(StringUtils.isNotBlank(req.getSearchType())){
				...
				res.setPaymentNoSearch("");
				res.setBrokerNameSearch("");
				res.setCompanyNameSearch("");
				res.setRemarksSearch("");
			}
		 ...
		 log.info("TreasuryDAOImpl getDirectCeding || Enter");
	        ...
	        try{
	            ...
	            for(int i=0 ; i<result.size(); i++) {
					...
					tempBean.setId(tempMap.get("ID")==null?"":tempMap.get("ID").toString());
					tempBean.setName(tempMap.get("NAME")==null?"":tempMap.get("NAME").toString());
		...
		/*	if(StringUtils.isBlank(bean.getFullSearch()) && StringUtils.isBlank(bean.getSearchType())){
			...
			for(int i = 0; i < list.size(); i++ ) {
				...
				}else 
				{
				 ...
				 if(StringUtils.isNotBlank(req.getOpStartDate())&& StringUtils.isNotBlank(req.getOpEndDate()) && StringUtils.isNotBlank(tempBean.getTrdate())){
