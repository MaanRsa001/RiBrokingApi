		...
		try{
			...
			if(resMap!=null){
				...
				if(!CollectionUtils.isEmpty(list1)) {
					res.setCeaseStatus(list1.get(0).get("CEASE_STATUS")==null?"":list1.get(0).get("CEASE_STATUS").toString());
				}
...
//					this.showSecondpageEditItems(beanObj, pid, proposalno);
	...
	public String[] savemodeUpdateRiskDetailsSecondFormSecondTable(
		...
		if ("3".equalsIgnoreCase(beanObj.getProductId()) || "5".equalsIgnoreCase(beanObj.getProductId())) {
			...
			obj[16] = StringUtils.isEmpty(beanObj.getGmsApproval()) ? "" : beanObj.getGmsApproval();
			...
			obj[34] = StringUtils.isEmpty(beanObj.getAmendId())?"":beanObj.getAmendId();
		}
		return obj;
	}
