...
//	                			from table(SPLIT_TEXT_FN(replace(PD.SPC,' ', '')))) AND  SPFC.TMAS_PRODUCT_ID = '4' AND PD.BRANCH_CODE = SPFC.BRANCH_CODE) PREMIUM_SUBCLASS_NAME
	...
	try {
		RskPremiumDetails entity = new RskPremiumDetails();
		    ...
		    entity.setVatPremiumOc(new BigDecimal(getModeOfTransaction(beanObj.getServiceTax(),beanObj)));;
		    entity.setVatPremiumDc(new BigDecimal(dropDowmImpl.GetDesginationCountry(entity.getVatPremiumOc().toString(), beanObj.getExchRate())));;
	...
	private  String getNetDueAmount( RskPremiumDetails entity, String CliamPaid) {
		...
		{
		Abt+=Double.parseDouble(entity.getVatPremiumOc().toString());
		}
