...
import com.maan.insurance.model.res.facPremium.getBrokerAndCedingNameRes;
import com.maan.insurance.model.res.facPremium.getBrokerAndCedingNameRes1;
...
public class FacPremiumServiceImple implements FacPremiumService {
	...
	public PremiumedListRes getPremiumedList(PremiumedListreq req) {
		...
		query = "premium.select.getPremiumedList";
		...
		for(int i=0 ; i<list.size() ; i++) {
			...
			if (!CollectionUtils.isEmpty(list1)) {
				tempBean.setAllocatedYN(list1.get(0).get("ALLOCATEDYN") == null ? ""
		...
		try {
				...
				String[] args = insertArguments(beanObj);
			 	...
			 	if("submit".equalsIgnoreCase(beanObj.getButtonStatus())){
			 		res.setTransactionNo(fm.getSequence("Premium",beanObj.getProductId(),beanObj.getDepartmentId(), beanObj.getBranchCode(),"",beanObj.getTransaction()));
					query = "FAC_TEMP_STATUS_UPDATE";
					args = new String[5];
			 		args[0] = "A";
			 		args[1] = beanObj.getLoginId();
			 		args[2] =res.getTransactionNo()==null?"":beanObj.getTransactionNo();
	...
	private String[] insertArguments(final PremiumInsertMethodReq beanObj)
	{
		String[] args=null;
		args=new String[53];
		args[0]=StringUtils.isBlank(beanObj.getReceiptno())?"":beanObj.getReceiptno();
	    args[1]=beanObj.getContNo();
		...
		args[9]=StringUtils.isBlank(beanObj.getCommissionview())?"":beanObj.getCommissionview();
	    ...
	    args[46]=beanObj.getSubProfitId().replace(" ", "");
			...
			args[16]="Y";
			args[18]="2";
			args[19]=StringUtils.isBlank(beanObj.getSettlementstatus())?"":beanObj.getSettlementstatus();
	...
	public  String[] updateAruguments(final PremiumInsertMethodReq beanObj) {
		...
		args[6]=StringUtils.isEmpty(beanObj.getCommissionview())?"":beanObj.getCommissionview();
	    ...
	    if(StringUtils.isBlank(beanObj.getTransactionNo())){
			...
			args[13]="Y";
			args[15]="2";
			args[16]=StringUtils.isEmpty(beanObj.getReceiptno())?"":beanObj.getReceiptno();
			...
			args[30]=StringUtils.isEmpty(beanObj.getAmendmentDate())?"":beanObj.getAmendmentDate();
		final String[] copiedArray = new String[args.length];
		System.arraycopy(args, 0, copiedArray, 0, args.length);
		return copiedArray;
	}
	...
	public getBrokerAndCedingNameRes getBrokerAndCedingName(String contNo, String branchCode) {
		...
		getBrokerAndCedingNameRes1  res = new getBrokerAndCedingNameRes1();
		List<getBrokerAndCedingNameRes1> reslist = new ArrayList<getBrokerAndCedingNameRes1>();
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
			try {
				String query = "broker.ceding.name";
				String args[]=null;
				args=new String[4];
				args[0]=contNo;
				args[1]=branchCode;
