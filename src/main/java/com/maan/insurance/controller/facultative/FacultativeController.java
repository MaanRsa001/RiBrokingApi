...
//		}
	...
	@PostMapping("/showFirstPageItems")
	public ShowFirstPageRes showFirstPageItems(@RequestBody ShowFirstPageReq req)throws CommonValidationException {
		List<ErrorCheck> error=facValidation.showFirstPageVali(req);
		if(error!=null && error.size()>0) {
				throw new CommonValidationException("error",error);
		}
			return facService.showFirstPageItems(req);
	}
