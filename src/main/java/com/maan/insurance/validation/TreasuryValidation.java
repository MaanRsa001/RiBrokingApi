...
//			}
	...
	public List<ErrorCheck> allocateDetailsVali(AllocateDetailsReq req) {
		...
		if (StringUtils.isBlank(req.getSerialNo())) {
			list.add(new ErrorCheck("Please Enter SerialNo", "SerialNo", "4"));
		}
		if (StringUtils.isBlank(req.getBranchCode())) {
	...
	public List<ErrorCheck> getTransContractVali(GetTransContractReq req) {
		GetTransContractListReq req1 = new GetTransContractListReq();
		List<ErrorCheck> list = new ArrayList<ErrorCheck>();
		if (StringUtils.isBlank(req.getAlloccurrencyId())) {
			list.add(new ErrorCheck("Please Enter AlloccurrencyId", "AlloccurrencyId", "1"));
		}
		...
		if (StringUtils.isBlank(req1.getReceivePayAmounts())) {
			list.add(new ErrorCheck("Please Enter ReceivePayAmounts", "ReceivePayAmounts", "4"));
		}
		if (StringUtils.isBlank(req1.getTransactionNo())) {
			list.add(new ErrorCheck("Please Enter TransactionNo", "TransactionNo", "5"));
		}
		if (StringUtils.isBlank(req.getBranchCode())) {
